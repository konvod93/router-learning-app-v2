...
const Users = () => {
  ...
  const handleUserClick = (userId) => {
    // Используем тот же механизм, что и в Link компоненте
    const path = `/users/${userId}`;
    window.history.pushState({}, "", path);
    window.dispatchEvent(new PopStateEvent("popstate")); // ← Ключевое!
  };
  ...
  return (
    <div className="bg-purple-50 p-6 rounded-lg">
      ...
      <p className="text-gray-700 mb-4">
        Список пользователей с динамическими ссылками. Кликните на любого
        пользователя для просмотра деталей.
      </p>
